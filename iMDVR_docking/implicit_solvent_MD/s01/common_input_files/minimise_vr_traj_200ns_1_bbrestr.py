...
restrain ca2+ ions. Add small backbone restraint. Run for 200ps or so as an initial check. 
...
pdb = "vr_binding_1/vr_binding_1_min_hbonds.pdb"
prmtop = AmberPrmtopFile('../new_6yb7_and_subs01.prmtop')
...
for inc in heating_increments:
    ...
    simulation.reporters.append(DCDReporter(f'binding_1_production_md_200ns_1/heat_{inc}_prod_6yb7_subs1.dcd', 1000))
...
for i in range(0, rounds):
    ...
    simulation.reporters.append(app.StateDataReporter(stdout, 5000, step=True,
                                                      ...
                                                      remainingTime=True,
                                                      speed=True, totalSteps=1000, separator='\t'))
    simulation.reporters.append(DCDReporter(f'binding_1_production_md_200ns_1/eq_{i}_prod_6yb7_subs1.dcd', 2000))
...
simulation.context.setPositions(positions)
simulation.reporters.append(PDBReporter('binding_1_production_md_200ns_1/output_6yb7_subs1.pdb', 1))
simulation.step(1)
...
with open("binding_1_production_md_200ns_1/output_6yb7_subs1.xml", 'w') as f:
    f.write(system_xml)
simulation.reporters.append(app.StateDataReporter(stdout, 10000, step=True,
                                                  potentialEnergy=True, temperature=True, progress=True,
                                                  remainingTime=True,
                                                  speed=True, totalSteps=1000, separator='\t'))
simulation.reporters.append(DCDReporter('binding_1_production_md_200ns_1/prod_6yb7_subs1.dcd', 50000))
simulation.step(100000000)
total_sim_time += 100000000 * 0.002
